...
def setup_db():
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS customers (
            ...
            total_funded_amount TEXT,
            total_interest_income TEXT,
            average_interest_rate TEXT
        )
    ''')
    conn.commit()
    conn.close()
...
class CRMApp:
    ...
    def create_create_contact_page(self):
        ...
        labels = ["Name", "Email", "Phone", "Title", "Company", "Nation", "City", "Address", "Zip Code", "Loan amount", "Interest Rate (%)", "Risk level", "Total loan amount", "Total funded amount", "Total interest income", "Average interest rate (%)"]
        self.entries = {}
    ...
    def create_view_contacts_page(self):
        ...
        self.contacts_tree = ttk.Treeview(frame, columns=("ID", "Name", "Email", "Phone", "Title", "Company", "Nation", "City", "Address", "Zip Code", "Loan amount", "Interest Rate (%)", "Risk level", "Total loan amount", "Total funded amount", "Total interest income", "Average interest rate (%)"), show="headings")
    ...
    def add_customer(self):
        ...
        if customer_data["name"]:
            ...
            c = conn.cursor()
            c.execute('''
                INSERT INTO customers (name, email, phone, title, company, nation, city, address, zip_code, loan_amount, interest_rate, risk level, )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
